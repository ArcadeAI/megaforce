...
from style_agent.llm_provider_setup import get_llm
from style_agent.prompt_builder import build_generation_prompt, build_related_content_prompt
...
async def generate_related_content(
    ...
    """Main interface for related content generation functionality with parallel processing.
...
async def process_related_content_schema(
    ...
    prompt = build_related_content_prompt(
        output_schema, reference_style, intent, focus, target_content, llm_provider
    )

    system_message = (
        "You are an expert content creator specializing in related content generation. "
    ...
    content = processed_content.model_dump_json(indent=2)
