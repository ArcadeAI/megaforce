...
export default function ApprovalQueue() {
  ...
  const submitTwitterPost = async () => {
    ...
    try {
      const result = await apiClient.postToTwitterByOutputId(twitterModal.output.id)

      // Update output status to "published" after successful posting (if backend didn't already)
      await apiClient.updateOutput(twitterModal.output.id, {
        status: 'published',
        published_url: result.tweet_url || (result.tweet_id ? `https://x.com/i/web/status/${result.tweet_id}` : undefined)
      })
          ...
          <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                />
                <p className="text-xs text-gray-400 mt-1">Scheduled time is interpreted using the configured app timezone.</p>
              </div>
            </div>
            ...
            <div className="flex justify-end space-x-3 mt-6">
              ...
              <Button
                onClick={async () => {
                  ...
                  try {
                    // Send the naive local datetime string; backend will interpret with configured timezone
                    const naive = new Date(scheduleTime)
                    const isoNaive = `${naive.getFullYear()}-${String(naive.getMonth()+1).padStart(2,'0')}-${String(naive.getDate()).padStart(2,'0')}T${String(naive.getHours()).padStart(2,'0')}:${String(naive.getMinutes()).padStart(2,'0')}:${String(naive.getSeconds()).padStart(2,'0')}`
                    await apiClient.scheduleOutput(scheduleModal.output.id, isoNaive)
