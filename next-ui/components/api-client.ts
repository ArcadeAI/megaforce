...
class ApiClient {
  ...
  async postToTwitterByOutputId(outputId: string): Promise<any> {
    const payload: any = { output_id: outputId }
  ...
  async connectAdminTwitter(): Promise<{ oauth_url: string; state: string; message?: string }> {
    // Initiate admin Twitter OAuth via settings endpoint
    return this.request('/api/v1/settings/twitter/connect', {
      method: 'POST',
      body: JSON.stringify({})
    })
  }
  ...
  async getSettings(): Promise<{ id: string; timezone: string; created_at: string; updated_at: string }> {
    return this.request('/api/v1/settings/')
  }

  async updateSettings(payload: { timezone: string }): Promise<{ id: string; timezone: string; created_at: string; updated_at: string }> {
    return this.request('/api/v1/settings/', {
      method: 'PUT',
      body: JSON.stringify(payload)
    })
